...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	assert(tree!=NULL);
	assert(val!=NULL);
	struct Node *searching = tree->root;
	while (searching!=NULL) {
		if (compare(searching->val,val) == 0) {
			return 1;
		}
		else if (compare(val,searching->val) == -1) {
...
void testContainsBSTree()
{
	...
	struct TestValues* notin = malloc(sizeof(struct TestValues));
	notin->n = 1;
	notin->values = malloc(notin->n * sizeof(struct data));
	initValue(&(notin->values[0]), 111, "not in tree");
    ...
    printTestResult(!(containsBSTree(tree, &(notin->values[0]))), "containsBSTree", "when test containing 111, which is not in the tree");
